...
class Raid():
    ...
    def pretty_display(self):
        if self.apinfo :
            a = PrettyTable([u"raid名字",u"sn", u"bios版本", u"fireware版本",u"bus id",u"device id",u"function id",u"使用软件"])
            a.add_row(
                [
    ...
    def format_pdinfo(self,output:str):
        ...
        for pd in j.get("Controllers")[0].get("Response Data").get("PD LIST"):
            ...
            if returncode == 0 :   # 如果磁盘状态bad,有可能无法查询到sn信息    
                # 有点曲折
                pdinfo.sn = json.loads(output1).get("Controllers")[0].get("Response Data").get("Drive {} - Detailed Information".format(pdposition)).get("Drive {} Device attributes".format(pdposition)).get("SN")
                pdinfo.manufacturer= json.loads(output1).get("Controllers")[0].get("Response Data").get("Drive {} - Detailed Information".format(pdposition)).get("Drive {} Device attributes".format(pdposition)).get("Manufacturer Id")
...
def get_PCIE_raid():
    ...
    if returncode == 0:
        ...
        for line in output.splitlines():
            if ('Serial Attached SCSI controller: Broadcom / LSI SAS3' in line) or ('Serial Attached SCSI controller: LSI Logic / Symbios Logic SAS3' in line): # sas3iru
